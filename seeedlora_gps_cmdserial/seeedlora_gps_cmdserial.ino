...
// Loop variable for sending LoRa package or anything that need a loop counting
  ...
  char c;
    ...
    while (!gps.location.isValid()) 
    {
      while (Serial2.available() > 0) 
      {
        if (gps.encode(c=Serial2.read())) 
        {
          if (gps.location.isValid()) 
          {
            ...
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print((String)"La: " + latitude);
            delay(1000);
            lcd.setCursor(0, 1);
            lcd.print((String)"Lo: " + longitude);
      ...
      if (millis() > 15000 && gps.charsProcessed() < 10)
      {
        ...
        Serial.println();                     
        Serial.println(F("Not able to get a fix in alloted time."));     
        break;
      }
    }
  #endif
  ...
  String command = Serial.readStringUntil('\n');
    ...
    **/
    if (command == "help") 
    {
        ...
        Serial.println("\tsend: send dummy data to FROST Server via LoRaWAN ");
        Serial.println();
    }
    ...
    **/
    else if (command == "send") 
    {
        ...
        lpp.addGPS(1, latitude, longitude, NULL);       // Encodes the GPS location data on channel 1 in Cayenne GPS format 
        ...
        if (Serial)
        {
            ...
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("Succesfully Sent");
            delay(2000);
            lcd.setCursor(0, 1);
            lcd.print((String)"Data Packet#: " + nloops);
            delay(2000);
        }
        
        lcd.clear();
        lcd.setCursor(0, 0);
        lcd.print("Next Command Pls");
        delay(2000);
        
        Serial.println();                     
        Serial.println("Type in the command box either 'help' or 'send' to continue ");       
        Serial.println();
    }
